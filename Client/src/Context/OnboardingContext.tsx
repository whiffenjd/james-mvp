...
const cleanFormData = (
  data: Partial<InvestorOnboardingPayload>
): Partial<InvestorOnboardingPayload> => {
  const cleaned = { ...data };
  ...
  if (cleaned.investorType === "individual") {
    ...
    if (cleaned.individualInvestorType === "high_net_worth") {
      ...
      cleaned.individualInvestorType === "self_certified_sophisticated_investor"
    ) {
...
export function OnboardingProvider({
  ...
  return (
    <OnboardingContext.Provider
      value={{ state, dispatch, nextStep, prevStep, updateFormData }}
    >
      {children}
    </OnboardingContext.Provider>
  );
