...
//         const mappedInvestors = initialData?.investors?.map((inv, index) => ({
//           id: inv.investorId,
//           name: inv.name,
//           amount: inv.amount,
//           files: [
...
interface FundModalProps {
  ...
  onClose?: () => void;
  onSubmit?: (data: SubmitData) => void;
  initialData?: Partial<FormData | FundDetail> | null;
  mode?: "create" | "edit";
}
...
const isFile = (obj: any): obj is File => {
  return (
    obj instanceof File ||
    (obj && typeof obj.name === "string" && typeof obj.size === "number")
  );
};
...
const FundModal: React.FC<FundModalProps> = ({
    ...
    if (mode === "edit" && initialData) {
      ...
      if (initialData.investors) {
        const mappedInvestors = initialData.investors.map((inv) => ({
          ...
          name: inv.name,
          amount:
            typeof inv.amount === "number" ? inv.amount : Number(inv.amount),
          files: inv.documentUrl
            ? [
                new File([], inv.documentUrl.split("/").pop() || `Document`, {
  ...
  const handleSubmit = async () => {
    ...
    console.log("Submitting form data:", { formData, documents, investors });
    ...
    try {
      ...
      const updateData = {
        ...
        investors: investors.map((investor) => ({
          ...
          name: investor.name,
          amount: investor.amount,
          documentUrl: investor.files[0]?.name || "",
          addedAt: new Date().toISOString(),
        })),
      };
      ...
      const investorDocuments: { [index: number]: File } = {};
      investors.forEach((investor, index) => {
        if (investor.files.length > 0) {
          investorDocuments[index] = investor.files[0];
        }
      });

      if (!initialData?.id) {
        throw new Error("Fund ID is required for update");
      }
                  ...
                  documents.map((doc, index) => (
                    ...
                    >
                      <div
                        className="flex items-center space-x-3 cursor-pointer"
                        onClick={() =>
                          setPreviewFile((doc?.file || doc) ?? null)
                        }
                      >
                        <FileText className="w-4 h-4 text-gray-500" />
                        <div>
                ...
                <div className="relative">
                  {files.length === 0 ? (
                    ...
                    >
                      <div className="rounded-[10px] px-4 py-3 flex justify-between items-center w-full">
                        <span
                          onClick={() => setPreviewFile(files[0] ?? null)}
                          className="text-sm truncate max-w-[80%]"
                        >
                          {files[0]?.name.length > 30
                            ? `${files[0]?.name.slice(0, 27)}...`
      ...
      {(previewFile || previewSrc) && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-lg font-semibold mb-4 text-theme-primary-text">
              Preview: {isFile(previewFile) ? previewFile.name : "Document"}
            </h2>

            {mode === "edit" && !isFile(previewFile) && previewSrc ? (
              <div className="flex-1 p-4 h-full">
                <iframe
                  src={previewSrc}
                  className="w-full h-[60vh] border-0 rounded"
